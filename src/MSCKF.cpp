...
MSCKF::MSCKF(ros::NodeHandle& pnh):
  is_gravity_set(false),  is_first_img(true),  nh(pnh) 
 {

  return;
}

bool MSCKF::loadParameters() 
{
  // Frame id
  ...
  for (int i = 18; i < 21; ++i)
   ...
   state_server.state_cov(21, 21) = 0;
...
void MSCKF::imuCallback( const sensor_msgs::ImuConstPtr& msg) 
{
...
/**
 * @function  采集200个的imu测量值
 *   把平均值作为陀螺仪的bias值
 * 　加速度值作为重力向量值的模长作为g　设定重力向量为(0,0,-g)
 */
...
bool MSCKF::resetCallback( std_srvs::Trigger::Request& req, std_srvs::Trigger::Response& res) 
{
...
void MSCKF::featureCallback(const CameraMeasurementConstPtr& msg)
{
...
void MSCKF::mocapOdomCallback(const nav_msgs::OdometryConstPtr& msg) 
{
...
void MSCKF::batchImuProcessing(const double& time_bound) 
{
...
/** @function 计算F、 G 和IMU状态
 *  @param time 时间戳，根据时间戳进行积分
 * 					m_gyro　陀螺仪测量值
 * 					m_acc　加速度测量值
 */
void MSCKF::processModel(const double& time, const Vector3d& m_gyro, const Vector3d& m_acc)
{
    //cout<<"go into  processModel funcation "<<endl;
  ...
  Matrix<double, 22, 22> Phi = Matrix<double, 22, 22>::Identity() +
  ...
  state_server.state_cov.block<22, 22>(0, 0) = Phi*state_server.state_cov.block<22, 22>(0, 0)*Phi.transpose() + Q;

  if (state_server.cam_states.size() > 0) 
  {
    state_server.state_cov.block(0, 22, 22, state_server.state_cov.cols()-22) =
      Phi * state_server.state_cov.block(0, 22, 22, state_server.state_cov.cols()-22);
      
    state_server.state_cov.block(22, 0, state_server.state_cov.rows()-22, 22) =
      state_server.state_cov.block(22, 0, state_server.state_cov.rows()-22, 22) * Phi.transpose();
  }
  ...
  state_server.imu_state.time = time;
   //cout<<"exit  processModel funcation "<<endl;
  return;
}
/** @function 通过龙格库塔法积分得到IMU状态变量值
 * 
 * 输入陀螺仪测量值、加速度测量值和时间量
 * 在 processModel() 中调用
 */
void MSCKF::predictNewState(const double& dt, const Vector3d& gyro,  const Vector3d& acc)
{
    state_server.imu_state.m_a=acc; 
    state_server.imu_state.m_w=gyro; 
...
/** @function 通过IMU位姿计算相机的位姿
 * 
 * 
 */
 void MSCKF::stateAugmentation(const double& time) 
{
      //cout<<"go into  stateAugmentation  funcation "<<endl;
  // update IMU pose matrix [R_ic   t_ic] to t+td
	state_server.imu_state.td+=IMUState::gyro_bias_noise;
	double th=0.005;
	if(state_server.imu_state.td>th)
	    state_server.imu_state.td=th;
	else if (state_server.imu_state.td < -th)
	  state_server.imu_state.td=-th;
	  else;
	Vector3d gyro = state_server.imu_state.m_w;
	Vector3d acc = state_server.imu_state.m_a;
	double td = state_server.imu_state.td;
	//predictNewState(state_server.imu_state.td,gyro,acc);
 
      Vector3d delta_v_I_G = (quaternionToRotation(state_server.imu_state.orientation).transpose()  *
      acc +state_server.imu_state.gravity) *td;
      state_server.imu_state.position += state_server.imu_state.velocity *td; //位置更新  这里为什么去掉了 0.5*a*dt^2
      state_server.imu_state.velocity += delta_v_I_G;//速度更新
       Vector3d  deltaX_theta = gyro*td;
      const Vector4d dq_imu =  smallAngleQuaternion(deltaX_theta);
...
//   J.block<3, 3>(3, 18) =  R_w_i.transpose();
   J.block<3, 3>(0, 15) =  Matrix3d::Identity();
    ...
    J.block<3, 1>(0, 21) << R_w_i.transpose()*gyro;
   J.block<3, 1>(3, 21) << R_w_i.transpose()*skewSymmetric(gyro)*t_c_i;//+state_server.imu_state.velocity;
  ...
  MatrixXd state_cov_fixed = (state_server.state_cov + state_server.state_cov.transpose()) / 2.0;
  state_server.state_cov = state_cov_fixed;
      //cout<<"exit  stateAugmentation  funcation "<<endl;
  return;
}

void MSCKF::addFeatureObservations( const CameraMeasurementConstPtr& msg) 
{
     // cout<<"go into  addFeatureObservations  funcation "<<endl;
...
// cout<<"exit  addFeatureObservations  funcation "<<endl;
  return;
}

/** @function 计算一个3D点在单个相机观测下的误差
 *   在 featureJacobian() 中调用
 *    输入 相机ID 和特征点ID   输出H_X 和 H_F矩阵
 */
void MSCKF::measurementJacobian(const StateIDType& cam_state_id, const FeatureIDType& feature_id
...
/** @function 计算一个3D点在M个相机观测下的误差
 * 
 *  在 pruneCamStateBuffer()  中调用
 */
...
{
 //cout<<"go into  featureJacobian  funcation "<<endl;
  ...
  r = A.transpose() * r_j;
 //cout<<"exit  featureJacobian  funcation "<<endl;
  return;
}
/** @function EKF 更新步骤
 * 
...
{
 //cout<<"go into  measurementUpdate  funcation "<<endl;
  ...
  if (//delta_x_imu.segment<3>(0).norm() > 0.15 ||   //delta_x_imu.segment<3>(3).norm() > 0.15 ||
           delta_x_imu.segment<3>(6).norm() > 0.5 ||      //delta_x_imu.segment<3>(9).norm() > 0.5 ||
  ...
  state_server.imu_state.td += delta_x_imu(21);
  cout<<"td: "<<state_server.imu_state.td<<endl;
  ...
  MatrixXd state_cov_fixed = (state_server.state_cov +  state_server.state_cov.transpose()) / 2.0;
  state_server.state_cov = state_cov_fixed;
   //cout<<"exit  measurementUpdate  funcation "<<endl;
  return;
}
...
void MSCKF::removeLostFeatures() 
{
 //cout<<"go into  removeLostFeatures  funcation ... "<<endl;
  ...
  MatrixXd H_x = MatrixXd::Zero(jacobian_row_size,
      22+6*state_server.cam_states.size());
  ...
  for (const auto& feature_id : processed_feature_ids)
    map_server.erase(feature_id);
 //cout<<"exit removeLostFeatures  funcation "<<endl;
  return;
}
 ...
 *  在 pruneCamStateBuffer()  中调用
 */
void MSCKF::findRedundantCamStates(
...
void MSCKF::pruneCamStateBuffer() {
 //cout<<"go into  pruneCamStateBuffer  funcation "<<endl;
 ...
 //cout<<"exit pruneCamStateBuffer  funcation "<<endl;
  return;
}

void MSCKF::onlineReset() {
...
void MSCKF::publish(const ros::Time& time) {

  // Convert the IMU frame to the body frame.
  const IMUState& imu_state = state_server.imu_state;
  Eigen::Isometry3d T_i_w = Eigen::Isometry3d::Identity();
  T_i_w.linear() = quaternionToRotation( imu_state.orientation).transpose(); T_i_w.translation() = imu_state.position;
